...
class HuggingFaceChatModel(ChatModel):
    def __init__(
        ...
        processor, model = self.get_processor_model(model_name, adapter_dir)
    ...
    @staticmethod
    def get_processor_model(model_name, adapter_dir=None):
    ...
    def get_processor_model(model_name, adapter_dir=None):
        ...
        if adapter_dir:
            if PeftModel is None:
                raise ImportError(
                    "peft is required when --adapter-dir is provided"
                )
...
class Idefics2ChatModel(HuggingFaceChatModel):
    ...
    def get_processor_model(model_name, adapter_dir=None):
        if adapter_dir:
            raise ValueError("LoRA adapter loading is only wired for LLaVA")
