...
ASSIGNMENT_VARIANT_REPORTS = \
	...
	$(RESULTS_DIR)/weldprompt-diverse-k3-class-report.xlsx \
	$(RESULTS_DIR)/weldprompt-diverse-k5-class-report.xlsx \
	$(RESULTS_DIR)/weldprompt-diverse-k7-class-report.xlsx \
	$(RESULTS_DIR)/weldprompt-diverse-k9-class-report.xlsx \
...
LORA_EVAL_GUIDS ?= data/guids_private.json
LORA_WELDPROMPT_RESULTS_DIR ?= results_lora_weldprompt
LORA_WELDPROMPT_ADAPTER ?= /ssd1/xueqili/lora/llava-realworld
LORA_WELDPROMPT_PORT ?= 56874
LORA_EPOCHS ?= 3
LORA_BATCH_SIZE ?= 1
LORA_GRAD_ACCUM ?= 8
LORA_LR ?= 2e-4
...
.PHONY: llava_baseline_class weldprompt_variants weldprompt_summary llava_dynamic_experiments \
	...
	lora_smoke_train lora_smoke_eval lora_prepare_balanced lora_train_balanced \
	lora_smoke_train_balanced lora_experiments_balanced \
	lora_weldprompt_server lora_weldprompt_best lora_weldprompt_reports
...
lora_reports:
	...
	-o $(LORA_RESULTS_DIR)/zero-shot-direct-private-class-report.xlsx
	$(PYTHON) scripts/test_classification.py \
	--guids $(LORA_EVAL_GUIDS) \
	--truth data/data \
	--pred $(LORA_RESULTS_DIR)/lora-realworld \
	-o $(LORA_RESULTS_DIR)/lora-realworld-private-class-report.xlsx
	$(PYTHON) scripts/summarize_class_reports.py \
	--reports \
	$(LORA_RESULTS_DIR)/zero-shot-direct-private-class-report.xlsx \
	$(LORA_RESULTS_DIR)/lora-realworld-private-class-report.xlsx \
	--out $(LORA_RESULTS_DIR)/lora-summary.csv

lora_experiments: lora_train lora_eval_base lora_eval_adapter lora_reports

lora_prepare_balanced:
	$(PYTHON) finetune/prepare_llava_lora_data.py \
	--outdir $(LORA_DATA_DIR) \
	--balance-train-labels \
	$(LORA_PREP_FLAGS)

lora_train_balanced: lora_prepare_balanced
	$(PYTHON) finetune/train_llava_lora.py \
	--model "$(LORA_MODEL)" \
	--train-jsonl $(LORA_DATA_DIR)/train.jsonl \
	--eval-jsonl $(LORA_DATA_DIR)/dev.jsonl \
...
lora_smoke_train_balanced: lora_prepare_balanced
	$(PYTHON) finetune/train_llava_lora.py \
	--model "$(LORA_MODEL)" \
	--train-jsonl $(LORA_DATA_DIR)/train.jsonl \
	--eval-jsonl $(LORA_DATA_DIR)/dev.jsonl \
...
lora_experiments_balanced: lora_train_balanced lora_eval_base lora_eval_adapter lora_reports

########################
# LORA + DYNAMIC WELDPROMPT
########################

lora_weldprompt_server:
	cd remote_scripts && $(PYTHON) vlm_server.py \
	--model "$(LORA_MODEL)" \
	--adapter-dir "$(LORA_WELDPROMPT_ADAPTER)" \
	--port $(LORA_WELDPROMPT_PORT)

$(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-diverse-k5-class-report.xlsx: $(RESULTS_DIR)/cot_data.json
	REMOTE_MODEL_API_URL=http://127.0.0.1:$(LORA_WELDPROMPT_PORT) RESULTS_ROOT=$(RESULTS_DIR) $(PYTHON) scripts/run_weldprompt_variant.py \
	--model "$(LORA_MODEL)" \
	--guids $(LORA_EVAL_GUIDS) \
	--cotdata $(RESULTS_DIR)/cot_data.json \
	--outdir $(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-diverse-k5 \
	--k 5 \
...
$(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-similarity-k9-class-report.xlsx: $(RESULTS_DIR)/cot_data.json
	REMOTE_MODEL_API_URL=http://127.0.0.1:$(LORA_WELDPROMPT_PORT) RESULTS_ROOT=$(RESULTS_DIR) $(PYTHON) scripts/run_weldprompt_variant.py \
	--model "$(LORA_MODEL)" \
	--guids $(LORA_EVAL_GUIDS) \
	--cotdata $(RESULTS_DIR)/cot_data.json \
	--outdir $(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-similarity-k9 \
	--k 9 \
...
lora_weldprompt_best: \
	$(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-diverse-k5-class-report.xlsx \
	$(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-similarity-k9-class-report.xlsx

lora_weldprompt_reports: lora_weldprompt_best
	$(PYTHON) scripts/summarize_class_reports.py \
	--reports \
	$(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-diverse-k5-class-report.xlsx \
	$(LORA_WELDPROMPT_RESULTS_DIR)/weldprompt-similarity-k9-class-report.xlsx \
	--out $(LORA_WELDPROMPT_RESULTS_DIR)/lora-weldprompt-summary.csv
